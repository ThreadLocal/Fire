...
var svg_number = 0;// 用于存储已上传的svg(暂时只处理联网报警系统)以更新点位
var svg_total_num = 0;//用于存储所有上传的svg(暂时只处理联网报警系统)以更新点位
// 以系统为基础发送楼栋所有相关信息请求
...
function detailResult(systemCode, area) {
	...
	if ($("#topLeftDiv").find(".fileInput").eq(0).text() == "") {
	...
	$.ajax({
				...
				success : function(e) {
					svg_number=0;
					svg_total_num=$(".fileInput.hasFloorPic").length + $(".newFileInput.hasFloorPic").length;
					var buildindgInform = e;// 获取修改的楼栋及楼层信息
					alert("上传成功");
					// 做反馈处理
					$("#upLoadBuild").attr("class", "noBuildPic");
					var newChangeInputDiv = $(".newFileInput.hasFloorPic");
					if($("#topLeftDiv").find(".fileInput").eq(0).text() == ""){//说明为新增楼栋选项
						...
						for ( var i = 0; i < newChangeInputDiv.length; i++) {
							...
							var file=newChangeInputDiv.eq(i).prop("files")[0];
							reader.readAsDataURL(file);
							(function(i){
								reader.onload=function(arg){;
							    var path=arg.target.result;
									$("#svg").load(path, function() {
										detailSvg(systemCode,buildindgInform["buildingID"],floorArray[i]);  //添加svg标准格式进svg重组数组;
									});	};	
							})(i);						
					...
					for ( var i = 0; i < oldChangeInputDiv.length; i++) {
						var reader=new FileReader();
						var file=oldChangeInputDiv.eq(i).prop("files")[0];
						reader.readAsDataURL(file);
						(function(i){
							reader.onload=function(arg){
							    var path=arg.target.result;							   
									$("#svg").load(path, function() {
										detailSvg(systemCode,buildindgInform["buildingID"],oldChangeInputDiv.eq(i).parent().prev().find("span").eq(0).text());  //添加svg标准格式进svg重组数组
									});	};	
						})(i);						
						allBuildingData[area][0][$("#topLeftDiv").find(".fileInput").eq(0).text()][0][oldChangeInputDiv.eq(i).parent().prev().find("span").eq(0).text()]=true;
					...
					if ($("#" + buildindgInform["area"]).find(
						...
						$(".buildingDiv[buildId="+buildindgInform['buildingID']+"]").attr("newPic",new_build_picture);
						$(".buildingDiv[buildId="+buildindgInform['buildingID']+"]").attr("buildName",buildindgInform["buildingName"]);
						$(".buildingDiv[buildId="+buildindgInform['buildingID']+"]").click(function(){ //为新增楼栋监听修改变动
							var area = $(this).attr("area");
										var buildId = $(this).attr("buildId");
										var builidingDiv_div = $(this);
										var buildName = $(this).attr("buildName");
...
//重组svg格式
function detailSvg(systemName, buidId, floorName){
	var positionInfo = {};
	positionInfo["point"] = [];
	positionInfo["point"][0] = systemName;
	positionInfo["point"][1] = buidId;
	positionInfo["point"][2] = floorName;
	positionInfo["point"][3] = {};
	var $equipText = $("#equipNum").find("text");
	var svg_element = document.getElementsByTagNameNS("http://www.w3.org/2000/svg","svg")[0];
	var svg_width = svg_element.width.animVal.value;
	var svg_height =svg_element.height.animVal.value;
	for ( var i = 0; i < $equipText.length; i++) {
		var $device = $equipText.eq(i);
		var deviceNum = $device.text();
		var positionLeft = $device.offset().left - 14;
		positionLeft=positionLeft/svg_width;		
		var positionTop = $device.offset().top;
		positionTop=positionTop/svg_height;
...
function noticeWindow(titleName, inform, div) {
	...
	if (inform[1] == "") {
		...
		$("#rightImageDiv").css(
				"background",
				"url('" + "../xf5.19/"+allBuildingData[inform[1]][0][inform[2]][3]+ "') no-repeat");
		$("#rightImageDiv").css("background-size", "cover");
	} else {
...
$(document).ready(function() {
	$("body").prepend("<div id='svg' style='width:688px;height:688px;visibility:hidden;position:absolute;'></div>");
