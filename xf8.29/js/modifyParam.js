...
function resetTable(tableId,tableData){
	...
	$(".edit").on("click",function(){
		var $this = $(this);
		noticeWindow("修改设备参数阈值","modifyLimit");
	...
	$(".code").on("click",function(){
		...
		codeInfo.push($this.parent().attr("nodeType"));
...
function noticeWindow(titleName,operation){
	...
	var formContent='<span style="color:red">*</span>'+'&nbsp设备编号：'+'<input  disabled = "disabled" name="fireFacilityID" type="text" value="" maxlength="10" class="fileInput" chname="设备编号" necessity="1">'+'<br>'
		+'<span style="color:red">*</span>'+'&nbsp设备类型：'+'<input name="nodeType" type="text" value="" class="fileInput" chname="设备类型" necessity="1">'+'<br>'
		...
		+'<span style="color:red">*</span>'+'&nbsp参数上限：'+'<input name="facilityParamUP" type="text value="" id="facilityParamUP"  class="fileInput" chname="参数上限" necessity="1">'
		+"<span style='color:red'></span>"+'<br>'
		+'<span style="color:red">*</span>'+'&nbsp参数下限：'+'<input name="facilityParamDown" type="text" value=""  id="facilityParamDown" class="fileInput" chname="参数下限" necessity="1">'
		+"<span style='color:red'></span>"+'<br>'
	...
	$("#newButtonDiv1").on("click",function(){
			...
			formData[0] = $editInput.eq(0).val();
			formData[1] = $editInput.eq(1).val();
			formData[5] = $editInput.eq(2).val();
			formData[6] = $editInput.eq(3).val();
			formData[7] = $editInput.eq(4).val();
			formData[8] = $editInput.eq(5).val();
			$editSelect.each(function(){
			...
			formData[2] = $editSelect.eq(0).children("option:selected").text();
			formData[3] = $editSelect.eq(1).children("option:selected").text();
			formData[4] = $editSelect.eq(2).children("option:selected").text();
			var $up = $("#facilityParamUP");
			var $down = $("#facilityParamDown");
			if(submitWarning == ""){
				if(parseFloat($up.prop("value")) + "" === $up.prop("value") && parseFloat($down.prop("value")) + "" === $down.prop("value") && parseFloat($up.prop("value")) > parseFloat($down.prop("value"))){
					$("#facilityParamUP").next().html("");
					$("#facilityParamDown").next().html("");
					ajaxForForm(operation);														}
				else{
					if(parseFloat($up.prop("value")) + "" !== $up.prop("value")){
						   $("#facilityParamUP").next().html("&nbsp&nbsp&nbsp&nbsp格式错误");
						}
					   if(parseFloat($down.prop("value")) + "" !== $down.prop("value")){
						   $("#facilityParamDown").next().html("&nbsp&nbsp&nbsp&nbsp格式错误");
						}
					   if(parseFloat($up.prop("value")) <= parseFloat($down.prop("value"))){
						   $("#facilityParamUP").next().html("&nbsp&nbsp&nbsp&nbsp上限不能低于下限");
						}
				}			   				
			}
			else {
